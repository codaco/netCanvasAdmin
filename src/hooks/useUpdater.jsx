...
export const checkEndpoint = (updateEndpoint, currentVersion) =>
  fetch(updateEndpoint)
    ...
    .then(({ name, body, assets }) => {
      ...
      console.info(
        `No update available (current: ${currentVersion}, latest: ${name}).`
      );
      return false;
    })
    .catch((error) => {
...
const useUpdater = (updateEndpoint, timeout = 0) => {
        ...
        message: (
          <div className="dialog-release-notes allow-text-selection">
            <p>
              ...
              the software may impact which protocols you are able to open in
              Architect.
            </p>
            <Markdown
  ...
  const checkForUpdate = async () => {
    console.log('checkForUpdate not implemented');
    return;
    ...
    //       <>
    //         <p>
    //           A new version of Architect is available. To upgrade, see the link
