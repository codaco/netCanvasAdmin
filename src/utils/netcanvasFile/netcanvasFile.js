...
import canUpgrade from "@/lib/protocol-validation/migrations/canUpgrade";
...
const verifyNetcanvas = (filePath, protocol) =>
  Promise.all([
    ...
    importNetcanvas(filePath).then(readProtocol),
  ])
...
const migrateNetcanvas = (
  filePath,
  newFilePath,
  targetVersion = APP_SCHEMA_VERSION
) =>
  importNetcanvas(filePath)
    .then((workingPath) =>
      readProtocol(workingPath)
        ...
        .then(([updatedProtocol, migrationSteps]) => {
          console.info("Migrated protocol", {
            migrationSteps,
            updatedProtocol,
          });
