...
import { extract, archive } from "@/utils/protocols/lib/archive";
import migrateProtocol from "@/lib/protocol-validation/migrations/migrateProtocol";
...
import { mockProtocolPath, mockProtocol } from "./helpers";

jest.mock("fs-extra");
jest.mock("@/utils/protocols/lib/archive");
jest.mock("@/lib/protocol-validation/migrations/migrateProtocol");
...
describe("netcanvasFile/netcanvasFile", () => {
  beforeEach(() => {
    ...
    uuid.mockImplementation(
      jest.fn(() => "809895df-bbd7-4c76-ac58-e6ada2625f9b")
    );
    ...
    pruneProtocol.mockImplementation((protocol = {}) =>
    ...
    revertNetcanvas.mockImplementation(({ savePath }) =>
      Promise.resolve(savePath)
    );
    commitNetcanvas.mockImplementation(({ savePath }) =>
      Promise.resolve(savePath)
    );
    ...
    importNetcanvas.mockImplementation((netcanvasPath) =>
      Promise.resolve(netcanvasPath)
    );
  ...
  describe("createNetcanvas(destinationPath)", () => {
    ...
    it("deploys the export and resolves to destinationPath", async () => {
      await expect(
        createNetcanvas("/dev/null/new/user/netcanvas/path")
  ...
  describe("checkSchemaVersion(protocol, schemaVersion [optional])", () => {
    it("returns errors.MissingSchemaVersion if no schema version in protocol", async () => {
      ...
      await expect(
        checkSchemaVersion("/dev/null/netcanvas/file")
    ...
    it("returns [, schemaVersionStates.OK] if protocol is a match", async () => {
      ...
      await expect(
        checkSchemaVersion("/dev/null/netcanvas/file", 3)
  ...
  describe("saveNetcanvas(workingPath, protocol, filePath)", () => {
    it("successful save", async () => {
      ...
      expect(createNetcanvasExport.mock.calls).toEqual([
        ["/dev/null/existing/working/path", mockProtocol],
      ]);
      expect(deployNetcanvas.mock.calls).toEqual([
  ...
  describe("verifyNetcanvas(filePath)", () => {
    ...
    it("Resolves to filePath if validation passes", async () => {
      ...
      await expect(
        verifyNetcanvas(mockProtocolPath, { schemaVersion: 4 })
