...
const downloadAndDecryptProtocols = async (tempDir: string): Promise<void> => {
  const encryptionKey = import.meta.env.PROTOCOL_ENCRYPTION_KEY;
  const encryptionIv = import.meta.env.PROTOCOL_ENCRYPTION_IV;
...
describe("Test protocols", () => {
  ...
  //   if (import.meta.env.CI && import.meta.env.SKIP_PROTOCOL_DOWNLOAD) {
