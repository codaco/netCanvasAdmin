...
const closeWindow = () => console.log("close window not implemented");

// Create a formatted date string that can be used in a filename (no illegal chars)
const dateWithSafeChars = (date, replaceWith = "-") =>
...
const ProtocolSummary = ({ data }) => {
  ...
  const dateString = `${dateWithSafeChars(
    ...
    "-"
  )} ${dateWithSafeChars(now.toLocaleTimeString(), ".")}`;
  const fileName = `${path.basename(
    filePath,
    ".netcanvas"
  )} Protocol Summary (Created ${dateString}).pdf`;

  const printPDF = async () => {
    console.log('printPDF not implemented');
    return; 
    const options = {
